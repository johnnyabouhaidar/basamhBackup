...
def extract_purchase_order_details(pdf_path):
    with open(pdf_path, 'rb') as file:
        ...
        rdd = re.search(r'Receiving Date:\s*([0-9/]+)', text2)
        rdd_str = rdd.group(1) if rdd else None

        exp = re.search(r'Cancellation Date   :\s*([0-9/]+)', text2)
...
po_number, ship_to, po_release_date, supplier,rdd,exp = extract_purchase_order_details(pdf_path)
# Load the Excel file
...
with pd.ExcelWriter(xlsx_path, mode='a', engine='openpyxl') as writer:
    ...
    ship_to = ship_to.split(' ')[0]
    po_info_df = pd.DataFrame({
        'Purchase Order Number': [po_number],
        'Ship To': [ship_to],
        'PO Release Date': [convert_date_format(po_release_date,'%d/%m/%y','%d.%m.%Y')],
        'RDD': [convert_date_format(rdd,'%d/%m/%y','%d.%m.%Y')],
        'Expiry': [convert_date_format(exp,'%d/%m/%y','%d.%m.%Y')],
