...
def remove_rows_tuning(xlsx_path):
    ...
    for i, row in df.iterrows():
        ...
        if skip != True and "___" not in str(row[0]) :#and str(row[8])!="nan":
            try:
                xx = int(row[2])
                modified_data.append(row.tolist())
            except:
...
po_str, shipto, aprrovalDate, rdd_date, expiry_date = extract_purchase_order_details(pdf_path)
