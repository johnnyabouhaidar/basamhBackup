...
pdf_path = r"330221_0027911619_MAK_00040.pdf"
...
#pdf_path = r"330221_0027835194_JED_00004.pdf"
#xlsx_path = "output.xlsx"
...
def modify_even_rows(xlsx_path):
    ...
    for i, row in df.iterrows():
        if len(row) > 0 and str(row[0])[0].isdigit() and ' ' in str(row[0]):
            ...
            if len(modified_row) ==10:
                modified_row.insert(1,"empty")
    ...
    for i, row in df.iterrows():
        ...
        if "Pack Size" in str(row[0]):
            ...
            if len(unit)>3:
                ...
                if "Status &" in str(row[0]):
                    unit = find_word_before_keyword(str(row[0]),"Status")
                if "% disc" in str(row[0]) or "% DISC" in str(row[0]):
...
def extract_purchase_order_details(pdf_path):
    with open(pdf_path, 'rb') as file:
        ...
        #lastpageminus2=reader.pages[-2]
        #textminus2 = lastpageminus2.extract_text()        
        ...
        try:
            ...
            try:
                ...
                lastpageminus2=reader.pages[-2]
                textminus2 = lastpageminus2.extract_text()  
                po_rdd = re.search(r'Expected Receiving Date:\s*([0-9/]+)', textminus2)
                po_rdd_str = po_rdd.group(1) if po_release_date else None

                po_exp = re.search(r'PO Cancellation Date   :\s*([0-9/]+)', textminus2)
                po_exp_str = po_exp.group(1) if po_release_date else None                        
        # Extract Supplier
