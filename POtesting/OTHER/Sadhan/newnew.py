...
pdf_path = r"1050-DC800.pdf"
csv_path1= r"output.xlsx"
...
try:
    ...
    mainDF = mainDF[mainDF.columns.intersection(['Article code','Quantity','Net price','Total net price'])]
...
try:
    mainDF['Total net price'] = mainDF['Total net price'].apply(lambda x: x.replace(" ",""))
except:
...
def extract_purchase_order_details(pdf_path):
    with open(pdf_path, 'rb') as file:
        ...
        last_page_minus = reader.pages[-2]
        last_page_minus_text = last_page_minus.extract_text()        
        # Combine the text from both pages
        text = first_page_text + "\n" + last_page_text + "\n" + last_page_minus_text
        print(text)
        ...
        try:
            ...
            expiry_match = re.search(r'(\d{2}/\d{2}/\d{2})', expiry_date_text)
            expiry = expiry_match.group(1) if expiry_match else None
...
with pd.ExcelWriter(csv_path1, mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
    ...
    try:
        po_info_df = pd.DataFrame({
            'Purchase Order Number': ","+str([tables[0]["Order no."][0]]),
